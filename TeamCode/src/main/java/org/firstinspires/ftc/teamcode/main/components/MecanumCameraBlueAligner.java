...
@Singleton
@Config
public class MecanumCameraBlueAligner extends Component {
    public static PIDCoefficients forwardPid = new PIDCoefficients();
    public static PIDCoefficients strafePid = new PIDCoefficients();

    private final PIDController forwardController = new PIDController(0.003, 0.04, 0);
    private final PIDController strafeController = new PIDController(0.003, 0.04, 0);
    private final MecanumBase mecanumBase;
    private final BlueCamera camera;

    private final double cameraXTarget = 160, cameraYTarget = 130;

    @Inject
    public MecanumCameraBlueAligner(MecanumBase mecanumBase, BlueCamera camera) {
