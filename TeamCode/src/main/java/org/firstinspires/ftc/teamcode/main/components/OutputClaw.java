...
public class OutputClaw extends Component {
    ...
    public OutputClaw(HardwareMap hardwareMap) {
        left = new CrazyServo(
                new SimpleServo(hardwareMap, RobotPorts.CHS2, 0, 180),
                1,
                0);
        right = new CrazyServo(
                new SimpleServo(hardwareMap, RobotPorts.EHS4, 0, 180),
                0,
                1
        );
        pitch = new CrazyServo(
                new SimpleServo(hardwareMap, RobotPorts.CHS0, 0, 180),
                1,
                0
        );
        yaw = new CrazyServo(
                new SimpleServo(hardwareMap, RobotPorts.CHS1, 0, 180),
                0.15,
                0.8);
        claw = new CrazyServo(
                new SimpleServo(hardwareMap, RobotPorts.EHS5, 0, 180),
                1,
                0.4);
    }
